% function [game,Q_free_overall_i,Q_free_overall_j,Q_overall_i,Q_overall_j] = agent_simulation_trajectory(board, player_i_level, player_j_level, simulation_count)
...
if rem(player_i_level, 2) == 1
    pol_Q = load(filename, strcat('policy_', num2str(player_i_level)), strcat('Q_', num2str(player_i_level)));
    fn = fieldnames(pol_Q);
    pol_i = pol_Q.(fn{1});
    Q_based_i = pol_Q.(fn{2});
else
    pol_Q = load(filename, strcat('policy_t_', num2str(player_i_level)), strcat('Q_t_', num2str(player_i_level)));
    fn = fieldnames(pol_Q);
    pol_i = pol_Q.(fn{1});
    Q_based_i = pol_Q.(fn{2});
end
if rem(player_j_level, 2) == 0
    pol_Q = load(filename, strcat('policy_', num2str(player_j_level)), strcat('Q_', num2str(player_j_level)));
    fn = fieldnames(pol_Q);
    pol_j = pol_Q.(fn{1});
    Q_based_j = pol_Q.(fn{2});
else
    pol_Q = load(filename, strcat('policy_t_', num2str(player_j_level)), strcat('Q_t_', num2str(player_j_level)));
    fn = fieldnames(pol_Q);
    pol_j = pol_Q.(fn{1});
    Q_based_j = pol_Q.(fn{2});
end
alpha_free = .1;
beta_free = .98;
beta_free_w = .05; %model-free beta for making a weight in a sigmoid function
...
rel_i = zeros(size(w_trj_i));
rel_j = zeros(size(w_trj_i));
w_free = zeros(size(w_trj_i));
Q_nrm_fctr = 100;
beta_boltz = 5; %boltzman coeficient
...
% keeping track of all Q-values!
t = 20 * simulation_count;
Q_free_overall_i = zeros(t,board_size^2,5);
Q_free_overall_j = zeros(size(Q_free_overall_i));

Q_overall_i = zeros(size(Q_free_overall_i));
Q_overall_j = zeros(size(Q_free_overall_j));
time = 1;

% initialize model-free Q-learning which updates by time
Q_free_i = ones(size(pol_i));
Q_free_i(pol_i < 1e-5) = 0;
Q_free_j = ones(size(pol_j));
Q_free_j(pol_j < 1e-5) = 0;
...
%     disp(idx_sim)
    ...
    while history(trial).mxmv ~= 0
        %normalize Q_free s
        Q_free_nrm_i = Q_free_i / max(max(Q_free_i)) * Q_nrm_fctr;
        Q_free_nrm_j = Q_free_j / max(max(Q_free_i)) * Q_nrm_fctr;
        
%         w_free_t =  1 / (1 + exp(-beta_free_w*time)) - .5; %weight for model based compared to model free
        w_free_t = 0;
        w_free(time) = w_free_t;
%         sprintf('i is %d\nj is %d\ns is %d', history(trial).i,history(trial).j,history(trial).s)
        Q_overall_i(time,:,:) = (1-w_free_t)*Q_based_i + w_free_t*Q_free_nrm_i;   %weighted policy 
%         disp(Q_free_nrm_i(history(trial).s,:))
%         disp(Q_based_i(history(trial).s,:))
%         disp(exp(beta_boltz*Q_based_i(history(trial).s,:))/sum(exp(beta_boltz*Q_based_i(history(trial).s,:))))
%         disp(exp(beta_boltz*Q_free_nrm_i(history(trial).s,:))/sum(exp(beta_boltz*Q_free_nrm_i(history(trial).s,:))))
%         disp(w_free_t)
%         disp(time)
        policy = squeeze(Q_overall_i(time,history(trial).s,:));
        policy = exp(beta_boltz*policy)/sum(exp(beta_boltz*policy));
        [sgt_act, trj_rel] = make_trj(board,board_size,history,Q_overall_i,Q_overall_j,c_act_i,c_act_j,time,trial,'i',rel_i,rel_j); %suggest an action based on the trajectories
%         disp('policy:')
%         disp(policy)

%         sprintf('Suggested act by trajectory: %d', sgt_act)
        w_path = zeros(size(policy));
        if sgt_act ~= 0
            w_path(sgt_act) = 1;
        end
        w_trj = 1 / (1 + exp(-beta_trj*time)) + s_w_trj * trj_rel - .5;
%         sprintf('trajectory weight: %d', w_trj)

        weights = w_trj * w_path + (1 - w_trj) * policy;
        history(trial).a_i = datasample(1:5,1,'Weight',weights);
%         sprintf('actual selected act: %d', history(trial).a_i)
        w_trj_i(time) = w_trj;
        rel_i(time) = trj_rel;
        
        Q_overall_j(time,:,:) = (1-w_free_t)*Q_based_j + w_free_t*Q_free_nrm_j;   %weighted policy
%         disp(Q_based_j(history(trial).s,:))
%         disp(exp(beta_boltz*Q_based_j(history(trial).s,:))/sum(exp(beta_boltz*Q_based_j(history(trial).s,:))))
%         
%         disp(exp(beta_boltz*Q_free_nrm_j(history(trial).s,:))/sum(exp(beta_boltz*Q_free_nrm_j(history(trial).s,:))))
%         disp(Q_free_nrm_j(history(trial).s,:))
% 
%         disp(w_free_t)
%         disp(time)
        policy = squeeze(Q_overall_j(time,history(trial).s,:));
        policy = exp(beta_boltz*policy)/sum(exp(beta_boltz*policy));
        [sgt_act, trj_rel] = make_trj(board,board_size,history,Q_overall_i,Q_overall_j,c_act_i,c_act_j,time,trial,'j',rel_i,rel_j); %suggest an action based on the trajectories
%         disp('policy:')
%         disp(policy)
...
%         sprintf('actual selected act: %d', history(trial).a_j)
        w_trj_j(time) = w_trj;
        rel_j(time) = trj_rel;
        ...
        % update Q_free s
        Q_free_i(history(trial).s,history(trial).a_i) = ...
            (1-alpha_free)*Q_free_i(history(trial).s,history(trial).a_i) ...
            + alpha_free*(history(trial).r_i + ...
            beta_free * max(Q_free_i(history(trial + 1).s,:)));
        Q_free_j(history(trial).s,history(trial).a_j) = ...
            (1-alpha_free)*Q_free_j(history(trial).s,history(trial).a_j) ...
            + alpha_free*(history(trial).r_j + ...
            beta_free * max(Q_free_j(history(trial + 1).s,:)));
        
        % keeping track of Q_free
        Q_free_overall_i(time,:,:) = Q_free_i;
        Q_free_overall_j(time,:,:) = Q_free_j;
...
Q_free_overall_i = Q_free_overall_i(1:time,:,:);
Q_free_overall_j = Q_free_overall_j(1:time,:,:);
Q_overall_i = Q_overall_i(1:time,:,:);
Q_overall_j = Q_overall_j(1:time,:,:);
% end
%% reward functions
...
%% Trajectories
function [suggested_action, trj_reliability] = make_trj(board,board_size,history,Q_overall_i,Q_overall_j,c_act_i,c_act_j,time,trial,player,rel_i,rel_j)
switch player
    case 'i'
        Q_overall = Q_overall_i;
        c_act = c_act_j;
        rel = rel_i;
    case 'j'
        Q_overall = Q_overall_j;
        c_act = c_act_i;
        rel = rel_j;
end
...
while H >= 2
    for idx_s = 1:size(states,1)
        ...
        policy = squeeze(Q_overall(time,s,:));
