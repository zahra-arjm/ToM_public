...
for board = boards(1:4)
    ...
    for level = 0:level_max
        ...
        act_eq = [1 2 4 3 5]; %it is only differnt in board D
        switch board
            case 'A'
                board_size = 9;
...
%save final normalized Q-values
        ...
        pol_trn = zeros(size(pol));
        Q_trn = zeros(size(Q_final));
        for s_i = 1:board_size
            for s_j = 1:board_size
                for act = 1:5
                    s = s_i + (s_j - 1) * board_size;
                    s_eq = pos_eq(s_j) + (pos_eq(s_i) - 1) * board_size;
                    pol_trn(s_eq,act_eq(act)) = pol(s,act);
                    Q_trn(s_eq,act_eq(act)) = Q_final(s,act);
                end
            end
        end
        % %        transposed condition
        eval([strcat('policy_t_', num2str(level)) '= pol_trn;']);
        save(filename, sprintf('policy_t_%d', level), '-append')
        % save final normalized Q-values
        eval([strcat('Q_t_', num2str(level)) '= Q_trn;']);
