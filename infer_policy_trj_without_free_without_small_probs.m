function posterior_probabilities = infer_policy_trj_without_free_without_small_probs(data_filename)
...
for player_cond = ['i','j']
    ...
    for sims_i = 1:size(sims,2) %loops through all simulations which contains multiple games
        ...
        for games_i = 1:size(games,2) %loops through all games
            for trial_i = 1:size(games(games_i).sims,2) - 1 %loops through all trials except last one
                ...
                for i_level = 2: n_levels %not including random level
                    ...
                    if weights(self_action) < 1e-2 %trying to avoid small probabilities
                        weights(self_action) = 1e-2;
                    end
