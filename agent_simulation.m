function [game,Q_free_overall_i,Q_free_overall_j,Q_overall_i,Q_overall_j] = agent_simulation(board, player_i_level, player_j_level, simulation_count)
...
% prob_det = .99999; %the chance of selecting the best response
alpha_free = .1;
beta_free = .98;
w = .5; %model-free weight compared to model-based
...
for count = 1:simulation_count
    ...
    while history(trial).mxmv ~= 0
        ...
        Q_overall_j(time,:,:) = (1-w)*pol_j + w*Q_free_j;   %weighted policy     
        policy = Q_overall_j(time,history(trial).s,:);
        ...
        weights(idx) = prob_det/length(idx);
        weights(setdiff(1:end,idx)) = (1-prob_det)/(5-length(idx));
%         if any(policy(setdiff(1:end,idx))) %checks if any other move is possible
%             weights(setdiff(1:end,idx)) = policy(setdiff(1:end,idx)); %select all other than BRs based on their probabilities
...
%         disp(policy)
        ...
        Q_overall_i(time,:,:) = (1-w)*pol_i + w*Q_free_i;   %weighted policy     
        policy = Q_overall_i(time,history(trial).s,:);
        %inferring player i's BR
        %we can do this, because our agents are only undecided between max of two
        %moves
        idx = find(policy == max(policy),5); %find at most 5 indices of maximum value
        %build the weights needed for selecting the action
        weights = zeros(1,5);
