...
%                 if any(isnan(pol(s,:))) %if the pol variable was Nan (divided by zero; because the state was impossible) in a row
%                     pol(s, :) = 0;
%                 end
                if ~all(pol(s,:) == 0) %if the pol variable was not zero in a row do the boltzman transformation
                ...
                if ~all(pol(s,:) == pol(s,1)) %if the player was not indifferent between all moves  
                    ...
                    if size(I,2) > 1 %if Q_1 has two maxima at a state
                        ...
                        [~, policy(t, s)] = max(pol(s, :));
                    end
                end
            end
            pol_t = pol; %keeping policy at time t
%             if abs(Q_diff(t)) < epsilon_q
%                 break
%             end
            ...
            if p_action_diff(t) < 1e-2 && abs(Q_diff(t)) < epsilon_q && conv_BR > 15 %stop if it has reached action and convergence
                break
            end
        end
        disp(board)
        ...
        p_Q_BR(idx).conv_BR = conv_BR;
        p_Q_BR(idx).p = p_action_diff(1:t);
        p_Q_BR(idx).Q = Q_diff(1:t);
        p_Q_BR(idx).t = t;
