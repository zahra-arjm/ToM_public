function games = agent_simulation_trajectory_without_free(board, player_i_level, player_j_level, simulation_count)
...
if rem(player_i_level, 2) == 1
    pol_Q = load(filename, strcat('policy_', num2str(player_i_level)));
    fn = fieldnames(pol_Q);
    pol_i = pol_Q.(fn{1});
else
    pol_Q = load(filename, strcat('policy_t_', num2str(player_i_level)));
    fn = fieldnames(pol_Q);
    pol_i = pol_Q.(fn{1});
end
if rem(player_j_level, 2) == 0
    pol_Q = load(filename, strcat('policy_', num2str(player_j_level)));
    fn = fieldnames(pol_Q);
    pol_j = pol_Q.(fn{1});
else
    pol_Q = load(filename, strcat('policy_t_', num2str(player_j_level)));
    fn = fieldnames(pol_Q);
    pol_j = pol_Q.(fn{1});
end
...
%     disp(idx_sim)
    ...
    while history(trial).mxmv ~= 0
        ...
        [sgt_act, trj_rel] = make_trj(board,board_size,history,pol_i,c_act_j,time,trial,'i',rel_i); %suggest an action based on the trajectories
%         disp('policy:')
%         disp(policy)
...
%         sprintf('Suggested act by trajectory: %d', sgt_act)
        ...
        [sgt_act, trj_rel] = make_trj(board,board_size,history,pol_j,c_act_i,time,trial,'j',rel_j); %suggest an action based on the trajectories
        w_path = zeros(size(policy));
        if sgt_act ~= 0
            w_path(sgt_act) = 1;
        end
